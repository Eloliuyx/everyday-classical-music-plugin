...
__export(main_exports, {
  default: () => EverydayClassicalMusicPlugin
});
...
var EverydayClassicalMusicPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.musicData = {};
  }
  ...
  async loadJsonData() {
    ...
    console.log("Loading JSON file from path:", jsonFilePath);
    if (jsonFilePath) {
      try {
        ...
        new import_obsidian.Notice("Failed to load JSON data.");
      }
    } else {
      new import_obsidian.Notice("JSON file path is not set.");
    }
  }
  async onFileCreate(file) {
    const fileCreationTime = (0, import_moment.default)(file.stat.ctime).valueOf();
...
${content.slice(propertyFieldsEndIndex).trim()}`;
  ...
  async removeLinksBeforeDate(cutoffDate) {
    const files = this.app.vault.getMarkdownFiles();
    const cutoff = (0, import_moment.default)(cutoffDate, "YYYY-MM-DD");
    for (const file of files) {
      if (this.isDailyNoteFile(file)) {
        const fileDate = (0, import_moment.default)(file.basename, "YYYY-MM-DD", true);
  ...
  isDailyNoteFile(file) {
    ...
    return (0, import_moment.default)(file.basename, dailyNoteFormat, true).isValid();
  }
};
var MyPluginSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Backfill Existing Notes").setDesc("If enabled, backfill all existing daily notes with classical music suggestions").addToggle((toggle) => toggle.setValue(this.plugin.settings.backfillExistingNotes).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Remove Links Before Date").setDesc("Select a date to remove all the links added before this date").addText((text) => text.setPlaceholder("YYYY-MM-DD").setValue(this.plugin.settings.removeLinksBeforeDate).onChange(async (value) => {
      this.plugin.settings.removeLinksBeforeDate = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).addButton((button) => {
      button.setButtonText("Remove Links").setCta().onClick(async () => {
        const cutoffDate = this.plugin.settings.removeLinksBeforeDate;
        if ((0, import_moment.default)(cutoffDate, "YYYY-MM-DD", true).isValid()) {
          await this.plugin.removeLinksBeforeDate(cutoffDate);
        } else {
          new import_obsidian.Notice("Please enter a valid date in YYYY-MM-DD format");
        }
      });
    });
    ...
    const koFiButton = buttonDiv.createEl("button", { text: "Feed the Markhor \u{1F98C}\u{1FABD}" });
