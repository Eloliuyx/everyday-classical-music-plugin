...
The release workflow builds from a version tag, runs automated checks, and generates GitHub artifact attestations for `main.js`, `manifest.json`, and `styles.css` before attaching those same files to a draft release. This workflow is introduced with 1.1.0; older releases do not have these attestations.
